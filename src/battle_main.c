...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, u32 ability, u32 holdEffect)
{
    ...
    if (holdEffect == HOLD_EFFECT_MACHO_BRACE || holdEffect == HOLD_EFFECT_POWER_ITEM)
        ...
        speed *= 2;
    else if (holdEffect == HOLD_EFFECT_OLD_WRAPPINGS && !IsDynamaxed(battler) && (gBattleMons[battler].species == SPECIES_PATRAT || gBattleMons[battler].species == SPECIES_WATCHOG))
        speed = (speed * 120) / 100;
...
void SetTypeBeforeUsingMove(u32 move, u32 battlerAtk)
{
    ...
    if ((gFieldStatuses & STATUS_FIELD_ION_DELUGE && moveType == TYPE_NORMAL)
        || (gStatuses3[battlerAtk] & STATUS3_CHARGED_UP && moveType == TYPE_NORMAL)
