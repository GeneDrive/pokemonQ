...
static const u8 sAbilitiesAffectedByMoldBreaker[] =
{
    ...
    [ABILITY_SEE_YOU_LATER] = 1,
    [ABILITY_LIGHT_DEVOURER] = 1,
};
...
{//                   Defender -->
 //  Attacker         Normal  Fighting Flying  Poison  Ground   Rock    Bug     Ghost   Steel  Mystery  Fire   Water   Grass  Electric Psychic   Ice   Dragon   Dark   Fairy   Time
    ...
    [TYPE_GHOST]    = {X(0.0), ______, ______, ______, ______, ______, ______, X(2.0), ______, ______, ______, ______, ______, ______, X(2.0), ______, ______, X(0.5), ______, X(2.0)},
#else
    [TYPE_GHOST]    = {X(0.0), ______, ______, ______, ______, ______, ______, X(2.0), X(0.5), ______, ______, ______, ______, ______, X(2.0), ______, ______, X(0.5), ______, X(2.0)},
#endif
    ...
    [TYPE_PSYCHIC]  = {______, X(2.0), ______, X(2.0), ______, ______, ______, ______, X(0.5), ______, ______, ______, ______, ______, X(0.5), ______, ______, X(0.0), X(2.0), X(0.5)},
...
#endif
    [TYPE_FAIRY]    = {______, X(2.0), ______, X(0.5), ______, ______, X(0.5), ______, X(0.5), ______, X(0.5), ______, ______, ______, X(0.5), ______, X(2.0), X(2.0), ______, X(0.0)},
    [TYPE_TIME]     = {______, ______, ______, ______, X(0.5), ______, ______, X(0.0), X(2.0), ______, ______, X(2.0), X(0.5), ______, ______, ______, X(0.5), ______, X(2.0), X(0.0)},
};
...
u8 DoBattlerEndTurnEffects(void)
{
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        case ENDTURN_LOOPER:
            if (gDisableStructs[battler].slowStartTimer
                && --gDisableStructs[battler].slowStartTimer == 0
                && ability == ABILITY_LOOPER
                && !BATTLER_MAX_HP(battler)
                && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK)
                && gBattleMons[battler].hp != 0)
            {
                gBattleMoveDamage = GetDrainedBigRootHp(battler, GetNonDynamaxMaxHP(battler) - gBattleMons[battler].hp);
                BattleScriptExecute(BattleScript_SlowStartEnds);
                BattleScriptExecute(BattleScript_IngrainTurnHeal);
                effect++;
            }
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_SLOW_START:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gDisableStructs[battler].slowStartTimer = 5 * 2;
        ...
        case ABILITY_LOOPER:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gDisableStructs[battler].slowStartTimer = 4 * 2;
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_SLOWSTART;
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                BattleScriptPushCursorAndCallback(BattleScript_SwitchInAbilityMsg);
                effect++;
    ...
    case ABILITYEFFECT_ABSORBING: // 3
        if (move != MOVE_NONE)
        {
            ...
            case ABILITY_LIGHT_DEVOURER:
                if (moveType == TYPE_FIRE || moveType == TYPE_ELECTRIC || moveType == TYPE_PSYCHIC || moveType == TYPE_FAIRY)
                    effect = 1;
                break;
            }
...
static inline u32 CalcAttackStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk)
{
    ...
    case HOLD_EFFECT_THICK_CLUB:
        if ((atkBaseSpeciesId == SPECIES_CUBONE || atkBaseSpeciesId == SPECIES_MAROWAK) && IS_MOVE_PHYSICAL(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_OLD_WRAPPINGS:
        if ((atkBaseSpeciesId == SPECIES_PATRAT || atkBaseSpeciesId == SPECIES_WATCHOG) && IS_MOVE_PHYSICAL(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
...
static inline uq4_12_t GetDefenderAbilitiesModifier(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, uq4_12_t typeEffectivenessModifier, u32 abilityDef)
{
    ...
    case ABILITY_PRISM_ARMOR:
        ...
        break;
    case ABILITY_THERMAL_EXCHANGE:
        if (typeEffectivenessModifier <= UQ_4_12(0.5) && moveType == TYPE_FIRE)
            return UQ_4_12(0.33);
        if (typeEffectivenessModifier == UQ_4_12(1.0) && moveType == TYPE_FIRE)
            return UQ_4_12(0.75);
        if (typeEffectivenessModifier == UQ_4_12(2.0) && moveType == TYPE_FIRE)
            return UQ_4_12(1.5);
        if (typeEffectivenessModifier == UQ_4_12(4.0) && moveType == TYPE_FIRE)
            return UQ_4_12(3.0);
        break;
    case ABILITY_FLUFFY:
